...
constexpr int64_t kSessionUs = 30LL * 60 * 1000 * 1000;
constexpr size_t kWallpaperLimit = 2 * 1024 * 1024;
constexpr size_t kMusicLimit = 16 * 1024 * 1024;
constexpr size_t kLrcLimit = 512 * 1024;
...
constexpr char kMediaUi[] = R"HTML(<script>(()=>{document.body.innerHTML=`<main><section class="hero"><h1>媒体管理</h1><p class="sub" id="subtitle">将文件安全传入设备 SD 卡。上传时请保持此页面打开。</p><span class="pill">局域网 · 30 分钟有效</span></section><section class="card"><div class="actions tabs"><button id="wallpaper">壁纸</button><button id="music" class="secondary">音乐</button></div><label class="drop" for="file"><strong id="uploadTitle">选择壁纸</strong><span id="picked">JPG 或 JPEG · 最大 2 MiB</span></label><input id="file" type="file"><div class="actions"><button id="upload">上传</button><button class="secondary" id="refresh">刷新列表</button></div><div class="progress"><div class="bar" id="bar"></div></div><div class="status" id="status">准备就绪</div></section><div class="section"><h2 id="listTitle">当前壁纸</h2><span id="count" class="pill">0 个文件</span></div><ul class="list" id="list"></ul></main><style>.tabs{margin-top:0}.tabs button{flex:1}.tabs button.active{background:#57c8ff;color:#03111e}</style>`;const $=s=>document.querySelector(s),file=$('#file'),status=$('#status'),bar=$('#bar');let type='wallpaper';const config={wallpaper:{title:'选择壁纸',list:'当前壁纸',hint:'JPG 或 JPEG · 最大 2 MiB',accept:'image/jpeg,.jpg,.jpeg',limit:2097152},music:{title:'选择音乐或歌词',list:'当前音乐',hint:'MP3 最大 16 MiB · LRC 最大 512 KiB',accept:'audio/mpeg,.mp3,.lrc',limit:16777216}};const say=(t,e=false)=>{status.textContent=t;status.className='status'+(e?' error':'')};const size=n=>n<1048576?(n/1024).toFixed(0)+' KB':(n/1048576).toFixed(2)+' MB';async function api(path,opt={}){const r=await fetch(path,opt),j=await r.json().catch(()=>({error:'设备响应无效'}));if(!r.ok)throw Error(j.error||'请求失败');return j}function choose(next){type=next;file.value='';$('#picked').textContent=config[type].hint;file.accept=config[type].accept;$('#uploadTitle').textContent=config[type].title;$('#listTitle').textContent=config[type].list;$('#wallpaper').className=type==='wallpaper'?'active':'';$('#music').className=type==='music'?'active':'secondary';bar.style.width='0%';say(type==='music'?'音乐播放或暂停时，设备会拒绝改动音乐文件。':'准备就绪');load()}async function load(){try{const j=await api('/api/list?type='+type),list=$('#list');list.innerHTML='';$('#count').textContent=j.files.length+' 个文件';if(!j.files.length){list.innerHTML='<li class="empty">还没有文件</li>';return}j.files.forEach(f=>{const li=document.createElement('li');li.className='file';li.innerHTML='<span class="thumb"></span><span class="name"></span><span class="size"></span>';li.querySelector('.name').textContent=f.name;li.querySelector('.size').textContent=size(f.size);const ren=document.createElement('button');ren.className='secondary';ren.textContent='重命名';ren.onclick=async()=>{const to=prompt('新文件名',f.name);if(to)try{await api('/api/rename?type='+type+'&from='+encodeURIComponent(f.name)+'&to='+encodeURIComponent(to),{method:'POST'});say('已重命名');load()}catch(e){say(e.message,true)}};const del=document.createElement('button');del.className='danger';del.textContent='删除';del.onclick=async()=>{if(confirm('确定删除“'+f.name+'”吗？'))try{await api('/api/file?type='+type+'&name='+encodeURIComponent(f.name),{method:'DELETE'});say('已删除');load()}catch(e){say(e.message,true)}};li.append(ren,del);list.append(li)})}catch(e){say(e.message,true)}}file.onchange=()=>{const f=file.files[0];$('#picked').textContent=f?f.name+' · '+size(f.size):config[type].hint};$('#wallpaper').onclick=()=>choose('wallpaper');$('#music').onclick=()=>choose('music');$('#refresh').onclick=load;$('#upload').onclick=()=>{const f=file.files[0];if(!f)return say('请先选择文件',true);const limit=type==='music'&&f.name.toLowerCase().endsWith('.lrc')?524288:config[type].limit;if(f.size>limit)return say('文件超过当前类型的大小限制',true);bar.style.width='0%';const xhr=new XMLHttpRequest();xhr.open('PUT','/api/upload?type='+type+'&name='+encodeURIComponent(f.name));xhr.upload.onprogress=e=>{if(e.lengthComputable){const p=Math.round(e.loaded/e.total*100);bar.style.width=p+'%';say('正在上传 '+p+'%')}};xhr.onload=()=>{let j={};try{j=JSON.parse(xhr.responseText)}catch{}if(xhr.status>=200&&xhr.status<300){bar.style.width='100%';say('上传完成，设备正在刷新媒体库');file.value='';$('#picked').textContent=config[type].hint;load()}else say(j.error||'上传失败',true)};xhr.onerror=()=>say('网络连接中断',true);xhr.send(f)};choose('wallpaper')})()</script>)HTML";
...
MediaTransferServer::MediaTransferServer(RefreshCallback refresh_wallpapers, RefreshCallback refresh_music,
                                         BusyCallback music_active, DisplayCallback show_qr, DisplayCallback hide_qr)
    : refresh_wallpapers_(std::move(refresh_wallpapers)), refresh_music_(std::move(refresh_music)),
      music_active_(std::move(music_active)), show_qr_(std::move(show_qr)), hide_qr_(std::move(hide_qr)) {
...
std::string MediaTransferServer::StartLocked(bool touch_owned, bool show_qr) {
    ...
    { std::lock_guard<std::mutex> filesystem_lock(SdCardManager::GetInstance().FilesystemMutex()); const std::string wallpapers = Directory(FileType::kWallpaper), music = Directory(FileType::kMusic); if ((mkdir(wallpapers.c_str(), 0775) != 0 && errno != EEXIST) || (mkdir(music.c_str(), 0775) != 0 && errno != EEXIST)) return "Unable to create media folders on the SD card."; }
    url_ = "http://" + WifiManager::GetInstance().GetIpAddress() + ":8080";
    httpd_config_t config = HTTPD_DEFAULT_CONFIG(); config.server_port = 8080; config.ctrl_port = 32770; config.stack_size = 12288; config.max_open_sockets = 4; config.uri_match_fn = httpd_uri_match_wildcard;
...
bool MediaTransferServer::IsSupportedFile(const std::string& name, FileType type) const { const std::string lower = Lower(name); return type == FileType::kWallpaper ? (HasSuffix(lower, ".jpg") || HasSuffix(lower, ".jpeg")) : (HasSuffix(lower, ".mp3") || HasSuffix(lower, ".lrc")); }
size_t MediaTransferServer::MaxFileSize(FileType type, const std::string& name) const { return type == FileType::kWallpaper ? kWallpaperLimit : HasSuffix(Lower(name), ".lrc") ? kLrcLimit : kMusicLimit; }
std::string MediaTransferServer::Directory(FileType type) const { return std::string(SdCardManager::GetInstance().MountPoint()) + (type == FileType::kWallpaper ? "/wallpapers" : "/music"); }
...
std::string MediaTransferServer::ListFiles(FileType type) const { std::lock_guard<std::mutex> filesystem_lock(SdCardManager::GetInstance().FilesystemMutex()); const std::string directory = Directory(type); DIR* handle = opendir(directory.c_str()); if (!handle) return "{\"files\":[]}"; std::vector<std::pair<std::string, size_t>> files; while (dirent* entry = readdir(handle)) { const std::string name = entry->d_name; if (!IsValidName(name) || !IsSupportedFile(name, type)) continue; struct stat info = {}; if (stat((directory + "/" + name).c_str(), &info) == 0 && S_ISREG(info.st_mode)) files.emplace_back(name, info.st_size); } closedir(handle); std::sort(files.begin(), files.end()); std::string json = "{\"files\":["; for (size_t i = 0; i < files.size(); ++i) { if (i) json += ','; json += "{\"name\":\"" + JsonEscape(files[i].first) + "\",\"size\":" + std::to_string(files[i].second) + '}'; } return json + "]}"; }

esp_err_t MediaTransferServer::GetHandler(httpd_req_t* req) { auto* self = static_cast<MediaTransferServer*>(req->user_ctx); ESP_LOGI(kTag, "GET %s", req->uri); if (PathEquals(req->uri, "/api/list")) { FileType type; if (!self->ParseFileType(req, type)) { self->SendError(req, 400, "Choose wallpaper or music."); return ESP_OK; } self->SendJson(req, 200, self->ListFiles(type)); return ESP_OK; } if (PathEquals(req->uri, "/")) { httpd_resp_set_type(req, "text/html; charset=utf-8"); httpd_resp_send_chunk(req, kHtml, HTTPD_RESP_USE_STRLEN); httpd_resp_send_chunk(req, kMediaUi, HTTPD_RESP_USE_STRLEN); httpd_resp_send_chunk(req, nullptr, 0); return ESP_OK; } self->SendError(req, 404, "Not found."); return ESP_OK; }

esp_err_t MediaTransferServer::UploadHandler(httpd_req_t* req) { auto* self = static_cast<MediaTransferServer*>(req->user_ctx); FileType type; const std::string name = self->ReadQuery(req, "name"); if (!self->ParseFileType(req, type) || !self->IsValidName(name) || !self->IsSupportedFile(name, type)) { self->SendError(req, 400, "Unsupported file type."); return ESP_OK; } if (type == FileType::kMusic && self->music_active_()) { self->SendError(req, 409, "Stop local music before changing music files."); return ESP_OK; } if (req->content_len <= 0 || static_cast<size_t>(req->content_len) > self->MaxFileSize(type, name)) { self->SendError(req, 400, "File is empty or exceeds this type's size limit."); return ESP_OK; } if (!SdCardManager::GetInstance().EnsureMounted().empty()) { self->SendError(req, 500, "SD card is unavailable."); return ESP_OK; } const std::string target = self->Directory(type) + "/" + name, part = target + ".part"; { std::lock_guard<std::mutex> filesystem_lock(SdCardManager::GetInstance().FilesystemMutex()); if (FILE* existing = fopen(target.c_str(), "rb")) { fclose(existing); self->SendError(req, 409, "A file with this name already exists."); return ESP_OK; } FILE* file = fopen(part.c_str(), "wb"); if (!file) { self->SendError(req, 500, "Cannot create file on SD card."); return ESP_OK; } std::array<char, 1024> buffer{}; int remaining = req->content_len; bool ok = true; while (remaining > 0) { const int received = httpd_req_recv(req, buffer.data(), std::min<int>(remaining, buffer.size())); if (received <= 0 || fwrite(buffer.data(), 1, received, file) != static_cast<size_t>(received)) { ok = false; break; } remaining -= received; } fclose(file); uint16_t width = 0, height = 0; FILE* validation = type == FileType::kWallpaper && ok && remaining == 0 ? fopen(part.c_str(), "rb") : nullptr; const bool valid_wallpaper = type != FileType::kWallpaper || (validation != nullptr && ReadJpegDimensions(validation, width, height) && width <= 1024 && height <= 1024); if (validation) fclose(validation); if (!ok || remaining != 0 || !valid_wallpaper || rename(part.c_str(), target.c_str()) != 0) { remove(part.c_str()); self->SendError(req, 400, type == FileType::kWallpaper ? "JPEG is invalid or exceeds the 1024 x 1024 pixel limit." : "Upload failed; no file was kept."); return ESP_OK; } } if (type == FileType::kWallpaper) self->refresh_wallpapers_(); else self->refresh_music_(); ESP_LOGI(kTag, "Uploaded %s: %s", type == FileType::kWallpaper ? "wallpaper" : "music", name.c_str()); self->SendJson(req, 200, "{\"ok\":true}"); return ESP_OK; }

esp_err_t MediaTransferServer::DeleteHandler(httpd_req_t* req) { auto* self = static_cast<MediaTransferServer*>(req->user_ctx); FileType type; const std::string name = self->ReadQuery(req, "name"); if (!self->ParseFileType(req, type) || !self->IsValidName(name) || !self->IsSupportedFile(name, type)) { self->SendError(req, 400, "Invalid file."); return ESP_OK; } if (type == FileType::kMusic && self->music_active_()) { self->SendError(req, 409, "Stop local music before changing music files."); return ESP_OK; } { std::lock_guard<std::mutex> filesystem_lock(SdCardManager::GetInstance().FilesystemMutex()); if (remove((self->Directory(type) + "/" + name).c_str()) != 0) { self->SendError(req, 404, "File was not found."); return ESP_OK; } } if (type == FileType::kWallpaper) self->refresh_wallpapers_(); else self->refresh_music_(); self->SendJson(req, 200, "{\"ok\":true}"); return ESP_OK; }

esp_err_t MediaTransferServer::RenameHandler(httpd_req_t* req) { auto* self = static_cast<MediaTransferServer*>(req->user_ctx); FileType type; const std::string from = self->ReadQuery(req, "from"), to = self->ReadQuery(req, "to"); if (!self->ParseFileType(req, type) || !self->IsValidName(from) || !self->IsValidName(to) || !self->IsSupportedFile(from, type) || !self->IsSupportedFile(to, type)) { self->SendError(req, 400, "New name has an unsupported extension."); return ESP_OK; } if (type == FileType::kMusic && self->music_active_()) { self->SendError(req, 409, "Stop local music before changing music files."); return ESP_OK; } { std::lock_guard<std::mutex> filesystem_lock(SdCardManager::GetInstance().FilesystemMutex()); const std::string directory = self->Directory(type); if (FILE* existing = fopen((directory + "/" + to).c_str(), "rb")) { fclose(existing); self->SendError(req, 409, "A file with this name already exists."); return ESP_OK; } if (rename((directory + "/" + from).c_str(), (directory + "/" + to).c_str()) != 0) { self->SendError(req, 404, "File was not found."); return ESP_OK; } } if (type == FileType::kWallpaper) self->refresh_wallpapers_(); else self->refresh_music_(); self->SendJson(req, 200, "{\"ok\":true}"); return ESP_OK; }
